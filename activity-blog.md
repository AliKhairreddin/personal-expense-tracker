# Activity Blog

## Planning Meeting 1

**Date:** 06/01/2026 
**Meeting Type:** Group planning meeting  
**Members Present:** 
Ali Khairreddin
Muhammad Wasay Munawar
Arshia Gole Sorkhi
Christian Elyasi
Jagshan Sangha
Hartaj Sangha
Ashrey Mathews
Alexander Ristic
Jazib Zaidi
Aryan Tyagi


### Main Discussion
The team discussed the Personal Expense Tracker project idea for CP476B. We agreed that the application will focus on helping students and young adults track expenses, organize spending categories, set budgets, and view simple spending summaries.

### Initial Task Assignments
- Wasay and Jagshan: Project proposal
- Arshia and Ashrey: Requirements and user stories
- Christian and Aryan: Wireframes
- Hartaj and Ali: Data planning
- Ali: Team plan
- Ali: GitHub setup
- Hartaj and Jazib: Activity blog/wiki

### Decisions Made
- The project will be a full-stack personal expense tracker.
- The app will use a relational database backend.
- The team will use GitHub for version control and project tracking.
- The GitHub Project board will use Backlog, Ready, In Progress, In Review, and Done columns.

### Next Steps
- Finish all Milestone 1 report sections.
- Finalize the Google Doc.
- Add GitHub links to links.txt.
- Export the report as PDF.
- Package the final submission ZIP.
